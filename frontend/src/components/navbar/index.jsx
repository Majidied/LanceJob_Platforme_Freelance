...
const Navbar = (props) => {
  ...
  const handleLogout = async () => {
    ...
    // Run the actual logout (e.g. clear tokens, call API, etc.)
    await logoutUser();

    // Keep spinner visible for at least 500ms
    setTimeout(() => {
      setLoggingOut(false);
      navigate("/login");
    }, 500);
              ...
              <div className="flex w-56 flex-col justify-start rounded-[20px] bg-white bg-cover bg-no-repeat shadow-xl shadow-shadow-500 dark:!bg-navy-700 dark:text-white dark:shadow-none">
                ...
                <div className="flex flex-col p-4">
                  ...
                  <button
                    ...
                    disabled={loggingOut}
                    className="flex items-center gap-2 mt-3 text-sm font-medium text-red-500 transition duration-150 ease-out hover:text-red-500 hover:ease-in"
                  >
                    {loggingOut ? (
                      <>
                        <svg
                          className="w-4 h-4 mr-2 text-red-500 animate-spin"
